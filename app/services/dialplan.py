...
from app.dto.dialplan import CreateDialplanDto, PutDialplanDto, DialplanStatusDto
...
class DialplanService:
    ...
    async def update_dialplan_status(self, client_id: str, data: DialplanStatusDto) -> tuple[dict, str]:
        try:
            async with self._db_session.begin():
                await self._dialplan_repository.update_dialplan_status(data.id, data.status)
                await self._dialplan_repository.update_task_status(data.id)
    ...
    async def get_task_id(self, client_id: str, dialplan_id: int) -> int:
        async with self._db_session.begin():
            return await self._dialplan_repository.get_task_id(dialplan_id)
